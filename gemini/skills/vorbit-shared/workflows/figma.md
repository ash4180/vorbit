...
4. **Load `figma-use` guidance before any `use_figma` call.** Try the `Skill` tool first; if `figma-use` is not registered, fall back to `ReadMcpResourceTool` with URI `skill://figma/figma-use/SKILL.md`. This guidance covers Plugin API gotchas that cause silent JavaScript failures.
