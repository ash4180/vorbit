...
4. **Load `figma-use` guidance before any `use_figma` call.** If `figma:figma-use` is in your available-skills list, call `Skill` with `figma:figma-use`. Otherwise call `ReadMcpResourceTool` with `server: figma` and `uri: skill://figma/figma-use/SKILL.md`. Both load the same content; pick whichever exists.
