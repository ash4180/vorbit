...
Preflight required connectors: confirm each needed connector is configured in Gemini CLI and inspect its current operation/parameter schemas; never guess tool names only when the user supplied an external artifact. Linear is the canonical PRD source; pasted acceptance criteria and local descriptions remain valid read-only inputs.
