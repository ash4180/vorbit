...
9. For each item, apply the normal implement workflow to that item only: search/reuse, mapped ACs/flows, honest focused test, implementation, regression checks. Move to In Progress and report the start in the current session.
...
12. After the queue, verify parent ACs. If covered, report "ready for verification/PR" in the current session, mark runtime state completed, and delete it. Otherwise persist `active: false`, `status: needs_input`, and the missing ACs rather than inventing work.
