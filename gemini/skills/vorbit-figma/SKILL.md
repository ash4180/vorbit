---
name: vorbit-figma
description: Use when the user wants to sync a Figma design system from code, use linked Figma design-system components/tokens, create Figma mockups from a PRD or journey, or plan front-end-ready Figma screens from complex flows.
---

# Vorbit Figma

Before working in Figma:

1. Read `../vorbit-shared/references/load-rules.md`.
2. Read `../vorbit-shared/workflows/figma.md`.
3. Load the applicable durable Vorbit rules for the current project and Gemini agent scope.
4. Then follow the Figma workflow: inspect code and Figma, map the linked design system, plan pages from the journey, confirm each phase with the user, and create design-system-backed Figma screens or libraries.
