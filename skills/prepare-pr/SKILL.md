...
5. **Detect lo-fi wireframes:**
   - Does `.claude/wireframes/` directory exist with any files?
   - If yes → Phase 3.5 will handle cleanup
   - If no → Skip Phase 3.5

6. **Check mock data (informational):**
   - Does `.claude/mock-registry.json` exist with entries?
   - If yes → Warn: "Mock data detected. Consider running `/cleanup-mocks` first."
   - Don't block — just inform

7. **Suggest review (informational):**
...
## Phase 3.5: Lo-fi Wireframe Cleanup

**Skip this phase if no `.claude/wireframes/` directory exists.**

**Goal**: Remove the lo-fi HTML wireframes generated by `/vorbit:design:explore` so they don't reach `dev`/`main`/`demo`. Unlike `.pen` design files, wireframes are ephemeral lo-fi artifacts — they don't need a recovery hash. If the explorer is rerun later, fresh wireframes are regenerated from the conversation context.

1. **Inventory wireframes** being cleaned:
   ```bash
   find .claude/wireframes -type f 2>/dev/null
   ```
   Record the topic-slug subfolders found.

2. **Remove tracked files (if any).** `.claude/wireframes/` should already be gitignored per the `/explore` Step 5b setup, but a leaked commit is possible:
   ```bash
   git rm -rf .claude/wireframes/ 2>/dev/null || true
   ```

3. **Remove untracked files** (the gitignored case — typical):
   ```bash
   rm -rf .claude/wireframes/
   ```

4. **Commit only if anything was tracked** — the `git status --porcelain` after step 2 tells you. If clean (gitignored case), skip the commit:
   ```bash
   git commit -m "chore: strip lo-fi wireframes before merge"
   ```
   Mechanical commit — no `Co-Authored-By`.

**Output**: Wireframes folder removed locally, and from git if it had been tracked.
...
2. **Analyze commit history** on this branch:
   ...
   ```
   Read ALL commits to understand the full scope of work. Group by type (feat, fix, refactor, chore). Ignore the "strip design files" and "strip lo-fi wireframes" commits — they're mechanical.
