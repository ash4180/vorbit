...
Correction Capture uses root causes. Routing uses types. They map to each other:

| Root cause | Routing type | Notes |
|---|---|---|
...
Corrections are stored as structured Obsidian notes in `~/Projects/Thinking-Labs/claude/projects/{project}/`. Each note has YAML frontmatter with: `date`, `project`, `project_path`, `capture_type`, `root_cause`, `status` (pending/done/cancelled), `rule`, `routed_to`, `tags`. The stop hook creates notes with `status: pending`; the agent updates them after classification.
