...
Generated by `stop_learn_reflect.py` Flow 2 (self-discovered learnings only). User-reported corrections (Flows 1/1b) now write directly to destination files on approval — they skip this file entirely. Each session block header contains the absolute project path for routing.
