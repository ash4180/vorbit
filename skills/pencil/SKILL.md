...
name: pencil
version: 1.0.0
description: Sync design tokens and components from a codebase to a Pencil canvas (`.pen` files), or set up a Pencil canvas from a style guide when no codebase exists. Use when the user says "sync pencil", "setup pencil", "configure pencil", "pencil sync", "sync tokens to pencil", "build pencil component library", or names Pencil/`.pen` files explicitly. Also triggers when mockups generated by Pencil don't match project conventions.
---
...
> **MCP namespace**: This skill uses `mcp__pencil__*` and optionally `mcp__mobbin__search_screens` for reference patterns. See `_shared/mcp-tool-routing.md` for the Plugin Tool Index and the announce-the-plugin rule.
...
> **Designs in git convention**: `.pen` files generated by this skill live in Pencil app storage. When the design is feature-specific and teammates should review it on a feature branch, **manually copy/export the `.pen` file to `designs/{issue-id}/<name>.pen` in the repo** and commit it. `/vorbit:implement:prepare-pr` strips that folder before merge (per the [Pencil Design Files in Git Management Standard](https://www.notion.so/vibranium-labs/Pencil-Design-Files-in-Git-Management-Standard-313477245840818dbf27dcc2d6774bde)). No skill auto-syncs Pencil app storage ↔ `designs/` — it's a manual step.
...
A Screen Shell is a reusable frame representing a real device screen with safe-area regions built in. Every mockup instances a Screen Shell, then inserts content into the `content` slot. See `references/build-recipes.md` for the full recipe — structure, per-platform safe-area values, build examples (iOS / Android / Web), the Screen Bone pattern (floating-nav and simple variants), the pre-build syntax checklist, and the batch strategy.
...
Before drawing each new screen, check if Mobbin MCP is available (`ToolSearch` for `"mobbin"`). If connected, follow `_shared/mobbin-research.md` for the synthesis format and "borrow concept, use library" rule. Skip when the screen is a direct re-skin of an existing component or when Mobbin is unavailable.
