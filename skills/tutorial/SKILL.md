---
name: tutorial
description: Use when the user asks to write a user tutorial, how-to guide, or user-facing help doc for a feature on the current branch. It finds the project's existing doc pattern (or asks for one), takes the flows from the branch prd.md (or from the user), captures one screenshot per step plus a short GIF from the running app when a browser capability exists (placeholders otherwise), writes the doc in plain language, and saves it next to the existing docs after approval. It may also post a Notion copy and add a link plus one approved comment to the Linear ticket. Do not use for QA plans, acceptance verification, code comments, or developer or API reference docs.
---
...
Read `../_shared/spec-files.md` for spec path resolution before any spec read.
...
1. Resolve the spec folder per `../_shared/spec-files.md`. Read `prd.md` when it exists: its user stories and flows become the tutorial sections and steps. Read `epic.md` for screen and route names when present.
...
2. **Notion (optional):** when a Notion connector is connected, ask once whether to post a copy. If yes, save using the "Save Content" section in `_shared/mcp-tool-routing.md` and pass the doc text; record the page URL. Images that cannot upload stay as links to the repo file; say so.
...
- Next: fill the placeholders and re-run this skill to revise, or run `/vorbit:implement:prepare-pr` to ship the doc with the branch
