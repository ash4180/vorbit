...
Preflight required connectors: confirm each needed connector is configured in Codex and inspect its current operation/parameter schemas; never guess tool names. Resolve the Figma connector before any write; if discovery or verification fails, tell the user to run your connector settings and stop.
