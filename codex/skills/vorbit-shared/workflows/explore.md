...
6. Analyze: summarize insights from all 10+ question answers, identify root cause (not symptoms), make ONE recommendation for direction (no multi-option voting ballot — explore is a partnership doc, not a vote).
7. **Draft exploration document in chat with this output shape (linear happy-path, single direction):**
   - **Context Summary** — key insights from 10+ Q&A. Constraints and Competitors sub-lines marked `(if any)` and omitted cleanly when empty.
   - **Problem Statement** — one sentence, root cause focus.
   - **User Flow** — happy path only, 3-8 plain-English numbered steps. Each step is one user action OR one system response (not both). Optional compact form: `[Start] → [Step 1] → [Step 2] → [Done]`.
   - **Reference Patterns** (when Step 3 ran) — anchor flow world named at the top; each block maps 1:1 to a User Flow step. Each block has a **Main screen reference** (`[App](URL)` from the anchor world) and 0-5 **Specific elements to borrow** bullets (blocks, buttons, copy lines, colors, micro-interactions). Cross-world borrows allowed for specific element bullets only, tagged `(Borrowed from [App](URL) — [reason anchor lacks this])`. Skip system-only flow steps (no screen).
