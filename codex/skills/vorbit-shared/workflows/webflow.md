<!-- GENERATED from skills/webflow/SKILL.md — edit the canonical file, then run: python3 -m vorbit_core.project_skills --write -->

> Skill assets: paths like `references/...` in this workflow resolve inside the installed `vorbit-webflow` skill directory (a sibling of `vorbit-shared`).
...
**Mode A - With Figma:**
Use the connected Figma MCP's `get_design_context` tool (resolve the connector per your connector preflight).
