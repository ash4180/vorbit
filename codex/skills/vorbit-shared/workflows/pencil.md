<!-- GENERATED from skills/pencil/SKILL.md — edit the canonical file, then run: python3 -m vorbit_core.project_skills --write -->

> Skill assets: paths like `references/...` in this workflow resolve inside the installed `vorbit-pencil` skill directory (a sibling of `vorbit-shared`).
...
- **Non-destructive**: Never modifies source code. Writes to the resolved Vorbit rules root and Pencil canvas only.

## Phase 0: Verify Pencil Connection

Preflight required connectors: confirm each needed connector is configured in Codex and inspect its current operation/parameter schemas; never guess tool names. Bare Pencil verbs in this skill (`get_editor_state`, `get_variables`, `batch_get`, `batch_design`, ...) refer to the connected Pencil MCP server's tools — resolve the current namespace through that discovery.

1. Check your configured connectors for `"pencil"` to check if Pencil MCP is available
2. **IF no Pencil tools found:** "No Pencil connection found. Configure the connector in Codex, then retry." → **STOP**
3. **IF found:** Call `get_editor_state` to verify connection
4. **IF fails:** "Pencil connection expired. Reconnect the connector in Codex, then retry." → **STOP**
...
Read the detection tables at `references/detection.md` from this skill's installed directory.

**Actions**:
1. Create todo list with all phases (0-6)
2. Check flags:
   - **`--refresh`:** Read existing `<rules-root>/projects/<project-slug>/pencil.md` (resolve the root via `vorbit-resolve-rules`), skip to Phase 2 using cached stack config. No confirmations until final sync preview.
...
5. **Detect platform and screen sizes** — two paths:
   ...
   **Step 1 — Ask platform** `[User]`:
   - **Use plain-text chat questions** to ask the user:
     ```
   ...
   **Step 2 — Select design system from Pencil** `[Pencil]`:
   ...
   2. **Use plain-text chat questions** `[User]` — present the tags and ask:
      ```
   ...
   4. **Use plain-text chat questions** `[User]` — show the style guide preview:
      ```
...
6. Present findings and **use plain-text chat questions** to confirm:
...
- The three actions that follow: sync tokens as Pencil variables, build reusable components on canvas (top 10), write project rules with screen presets to `<rules-root>/projects/<project-slug>/pencil.md` (resolve the root via `vorbit-resolve-rules`)
...
**Use plain-text chat questions** with this preview (Path A only). User can adjust what gets synced.
...
**First**, read `references/layout-model.md` from this skill's installed directory — the CSS→Pencil mental model. This teaches how block vs inline elements work, when to wrap text in frames, how flex layout maps to Pencil, and platform safe area rules. Understanding this prevents layout bugs at the source.
...
**Goal**: Write `<rules-root>/projects/<project-slug>/pencil.md` (resolve the root via `vorbit-resolve-rules`) with both code generation AND Pencil mockup rules.

Read `references/project-rules-output.md` from this skill's installed directory now. Fill its required template from the confirmed stack, extracted tokens, screen shells, and real Pencil component IDs; never leave bracket placeholders or example values in the written file. Write the result to `<rules-root>/projects/<project-slug>/pencil.md` (resolve the root via `vorbit-resolve-rules`), report the exact destination and sync counts, then mark all todos complete.

## Refresh Mode (`--refresh`)

1. Read existing `<rules-root>/projects/<project-slug>/pencil.md` (resolve the root via `vorbit-resolve-rules`) for cached stack config
...
6. Update rules file
7. **No plain-text chat questions stops** — refresh is meant to be fast

## Components-Only Mode (`--components-only`)

1. Read existing `<rules-root>/projects/<project-slug>/pencil.md` (resolve the root via `vorbit-resolve-rules`) for cached stack + tokens
