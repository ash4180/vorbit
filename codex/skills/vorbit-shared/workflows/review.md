<!-- GENERATED from skills/review/SKILL.md — edit the canonical file, then run: python3 -m vorbit_core.project_skills --write -->

> Skill assets: paths like `references/...` in this workflow resolve inside the installed `vorbit-code-review` skill directory (a sibling of `vorbit-shared`).
...
Detailed pipeline specs live in `references/` within this skill's directory. They ship inside this skill's installed directory (see the asset note above).
...
1. Read `./AGENTS.md` and other repository instruction files for project standards
2. Read `<rules-root>/projects/<project-slug>/review-rules.md` (resolve the root via `vorbit-resolve-rules`) if it exists — learnable rules from previous reviews
...
Read the pipeline spec (in this skill's `references/` directory) and execute all three layers in order:
...
- **No `<rules-root>/projects/<project-slug>/review-rules.md` (resolve the root via `vorbit-resolve-rules`)** → "No review rules file yet" (normal for first run)
