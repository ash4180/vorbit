...
# implement-loop is the only hand-written agent workflow (its Claude
# implementation depends on a Stop hook other runtimes don't have); every
# other workflow is generated by vorbit_core/project_skills.py.
...
def test_agent_workflow_sets_match():
...
def test_projected_outputs_are_fresh():
    """Every projected workflow, contract copy, and asset mirror matches the
    canonical source — the single-source-of-truth guarantee. Regenerate with
    `python3 -m vorbit_core.project_skills --write`."""
    from vorbit_core.project_skills import check_all
